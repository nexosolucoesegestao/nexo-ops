...
const API = {
  ...
  // ---- HEADERS: usa JWT do Supabase Auth quando disponível ----
  _headers() {
    var token = NEXO.SUPABASE_KEY; // fallback: anon/publishable key
    try {
      var projectId = NEXO.SUPABASE_URL.split('//')[1].split('.')[0];
      var stored = localStorage.getItem('sb-' + projectId + '-auth-token');
      if (stored) {
  ...
  async select(table, params, cacheMin) {
    ...
    try {
      ...
      if (cacheMin) {
        ...
        this._cacheTTL[cacheKey] = Date.now() + (cacheMin * 60000);
      }
  ...
  async insert(table, row) {
    ...
    try {
      var res = await fetch(this._url(table), {
        method:  'POST',
      ...
      if (!res.ok) {
        var err = await res.json();
        return { success: false, error: err.message || err.details || 'Erro ' + res.status };
      }
      var data = await res.json();
  ...
  async update(table, match, updates) {
    var query = Object.entries(match).map(function(e) {
      return e[0] + '=eq.' + encodeURIComponent(e[1]);
    }).join('&');
    try {
      var res = await fetch(this._url(table, query), {
        method:  'PATCH',
        headers: this._headers(),
  ...
  // ---- LOGIN: gerenciado pelo Auth.js via Supabase Auth ----
  async login(usuario) {
    // Mantido para compatibilidade com a camada de get/post
    // A autenticação real ocorre em Auth.login() via Supabase Auth
    return { success: true, data: {} };
  },

  // ---- QUERIES ESPECÍFICAS DO NEXO ----
  async getProdutosLoja(lojaId) {
    var res = await this.select('loja_produtos', {
      'select':   'produto_id,produtos(*)',
      'loja_id':  'eq.' + lojaId
    }, 60);
    if (!res.success) return res;
    var produtos = res.data.map(function(lp) { return lp.produtos; })
      .filter(Boolean).map(API._mapProduto);
    return { success: true, data: produtos };
  },

  async getPessoasLoja(lojaId) {
    var res = await this.select('pessoas', {
      'loja_id': 'eq.' + lojaId,
      'status':  'eq.ATIVO',
      'order':   'tipo,nome'
    }, 30);
  ...
  async getDadosDia(lojaId, data) {
    var regRes = await this.select('registros', {
      'loja_id': 'eq.' + lojaId,
      'data':    'eq.' + data
    });
    ...
    return {
      ...
      data: {
        ...
        temperatura:    (results[3].data && results[3].data.length > 0)
                          ? results[3].data[results[3].data.length - 1]
                          : null
      }
    };
  },
  ...
  async getDashboard(lojaId, dias) {
    ...
    if (regs.length === 0) {
      var empty = {
        periodo_dias: dias, coletas_realizadas: 0, score: 0,
        direcionamento: 'Nenhuma coleta nos últimos ' + dias + ' dias. Faça o check-in para começar.',
        disponibilidade: { percentual: 0, rupturas: 0, total_checks: 0 },
        top_rupturas: [],
        quebra:      { total_kg: 0, meta_kg: 10 },
        temperatura: { media: 'N/A', conformidade_pct: 0 },
        presenca:    { percentual: 0, presentes: 0, total: 0 },
        ocorrencias_ativas: 0, ocorrencias: []
      };
    ...
    var ruptPorProd = {};
    disps.filter(function(d) { return !d.tem_estoque; }).forEach(function(d) {
      ruptPorProd[d.produto_id] = (ruptPorProd[d.produto_id] || 0) + 1;
    });
    var topRupturas = [];
    ...
    if (temps.length > 0) {
      var soma = 0;
      temps.forEach(function(t) { soma += parseFloat(t.balcao_refrigerado) || 0; });
    ...
    var dashData = {
      periodo_dias: dias, coletas_realizadas: regs.length, score: score,
  ...
  async saveDisponibilidade(regId, produtos, lojaId) {
    ...
    var promises = produtos.map(function(prod) {
      ...
      return API.insert('disponibilidade', {
        ...
        tem_estoque: prod.tem_estoque === 'SIM', motivo_ruptura: prod.motivo_ruptura || '',
        disponivel_at: prod.disponivel_at === 'SIM', motivo_indisponivel_at: prod.motivo_indisponivel_at || '',
        disponivel_as: prod.disponivel_as === 'SIM', motivo_indisponivel_as: prod.motivo_indisponivel_as || ''
      });
    });
    await Promise.all(promises);
  ...
  async saveTemperatura(regId, payload) {
    ...
    var balcao = parseFloat(payload.balcao);
    var resf   = parseFloat(payload.camara_resfriados);
    var cong   = parseFloat(payload.camara_congelados);
  ...
  async saveAcaoOcorrencia(ocId, acao, pessoaId) {
    ...
    await this.insert('ocorrencias_acoes', { id: id, ocorrencia_id: ocId, acao_tomada: acao, pessoa_id: pessoaId });
  ...
  _enqueue(item) {
    ...
    this._saveQueue();
    Utils.toast('Salvo offline.', 'warning');
  },
  ...
  async _flushQueue() {
    ...
    for (var i = 0; i < pending.length; i++) {
      try { await this.insert(pending[i].table, pending[i].row); }
      catch(e) { this._queue.push(pending[i]); }
    }
    this._saveQueue();
  ...
  _saveQueue() {
    try { localStorage.setItem('nexo_offline_queue', JSON.stringify(this._queue)); } catch(e){}
  },

  _loadQueue() {
    try { this._queue = JSON.parse(localStorage.getItem('nexo_offline_queue') || '[]'); }
    catch(e){ this._queue = []; }
  },
  ...
  async get(action, params, cacheMin) {
    ...
    switch (action) {
      case 'GET_DASHBOARD':         return this.getDashboard(lojaId, dias);
      case 'GET_PRODUTOS':          return this.getProdutosLoja(lojaId);
      case 'GET_PESSOAS':           return this.getPessoasLoja(lojaId);
      case 'GET_MOTIVOS':           return this.getMotivos((params && params.contexto) || '');
      case 'GET_ACOES_PREDEFINIDAS':return this.getAcoesPredefinidas((params && params.tipo) || '');
      case 'GET_OCORRENCIAS':       return this.getOcorrencias(lojaId, (params && params.status) || '');
      case 'GET_DADOS_DIA':         return this.getDadosDia(lojaId, (params && params.data) || new Date().toISOString().split('T')[0]);
      case 'GET_REGISTROS':         return this.select('registros', { 'loja_id': 'eq.' + lojaId, 'order': 'data.desc' });
      case 'LOGIN':                 return this.login((params && params.usuario) || '');
      default:
    ...
    switch (action) {
      case 'SAVE_CHECKIN':
        return this.saveCheckin({
          loja_id: payload.id_loja || lojaId, pessoa_id: payload.id_pessoa || Auth.userId,
          hora_checkin: payload.hora_checkin || '', foto_url: payload.foto_url || '',
          observacao: payload.observacao || '', geo_lat: payload.geo_lat || '', geo_lng: payload.geo_lng || ''
        });
      case 'SAVE_CHECKOUT':         return this.saveCheckout(payload.id_registro || regId, payload.hora_checkout || '');
      case 'SAVE_PRESENCA':         return this.savePresenca(payload.id_registro || regId, payload.registros || []);
      case 'SAVE_DISPONIBILIDADE':  return this.saveDisponibilidade(payload.id_registro || regId, payload.produtos || [], lojaId);
      case 'SAVE_QUEBRA':           return this.saveQuebra(payload.id_registro || regId, payload.itens || []);
      case 'SAVE_TEMPERATURA':      return this.saveTemperatura(payload.id_registro || regId, payload);
      case 'SAVE_PESSOA':           return this.savePessoa(payload);
      case 'SAVE_ACAO_OCORRENCIA':  return this.saveAcaoOcorrencia(payload.id_ocorrencia, payload.acao, payload.id_pessoa || Auth.userId);
      case 'RESOLVER_OCORRENCIA':   return this.resolverOcorrencia(payload.id_ocorrencia, payload.motivo_resolucao);
      case 'UPLOAD_FOTO':           return this.uploadFoto(payload.base64, payload.filename);
      default:
