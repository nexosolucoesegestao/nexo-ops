// ============================================================
// NEXO OPS — Home v1.2
// Home limpa: só direcionamento + módulos
// Usa cache, pré-carrega dados estáticos
// ============================================================
Router.register('home', async function(app) {
  var checkinDone = Utils.isModuleDone('checkin');

  // Renderizar layout imediato (sem esperar API)
  app.innerHTML =
    '<div class="page">' +
      ...
      '<div class="home-greeting">' +
        '<div><p class="greeting-sub">' + Utils.getSaudacao() + ',</p><p class="greeting-name">' + (Auth.user.nome || 'Encarregado') + '</p><p class="greeting-loja">' + (Auth.loja.nome || '') + '</p></div>' +
        '<div class="score-ring" id="scoreRing"><span class="score-value">--</span><span class="score-badge">SCORE</span></div>' +
      '</div>' +
      '<div id="iaCard" class="ia-card"><div class="ia-card-inner"><div class="ia-header"><svg class="ia-icon" viewBox="0 0 16 16" fill="none"><path d="M8 1L10 6H15L11 9L12.5 14L8 11L3.5 14L5 9L1 6H6L8 1Z" fill="#C5A35A"/></svg><span>Direcionamento do dia</span></div><p class="ia-text" style="color:var(--gray-300)">Carregando análise...</p></div></div>' +
      '<div class="section-label">Coleta de hoje — 5 módulos</div>' +
      '<div class="modulos-list" id="modulosList"></div>' +
      buildNavBar('home') +
    '</div>';

  // Renderizar módulos IMEDIATAMENTE (não depende da API)
  renderModulos(checkinDone);

  // Pré-carregar dados estáticos em background (1x, com cache longo)
  API.preloadStaticData();

  // Buscar dashboard (com cache de 5 min)
  var dashRes = await API.get('GET_DASHBOARD', { loja_id: Auth.lojaId, dias: '7' }, 5);
  ...
  if (dashRes.success) {
    ...
    // Card IA
    var iaCard = document.getElementById('iaCard');
    if (iaCard) {
      iaCard.innerHTML =
        '<div class="ia-card-inner">' +
          ...
          '<div class="ia-actions">' +
            ...
            '<button class="btn-ia gold" onclick="Router.navigate(\'ocorrencias\')"><svg viewBox="0 0 14 14" fill="none"><rect x="2" y="2" width="10" height="10" rx="2" stroke="currentColor" stroke-width="1"/><path d="M5 7H9M7 5V9" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>Ocorrências (' + (d.ocorrencias_ativas || 0) + ')</button>' +
          '</div>' +
        '</div>';
    }
  }

  function renderModulos(checkinDone) {
    var modulos = [
    ...
    var list = document.getElementById('modulosList');
    if (!list) return;
    list.innerHTML = '';

    modulos.forEach(function(mod, i) {
      var done = Utils.isModuleDone(mod.id);
      var canOpen = (i === 0) || checkinDone;
      var isActive = canOpen && !done;
      var isReopen = canOpen && done;
      ...
      if (isActive || isReopen) {
        ...
        card.addEventListener('click', function() { Router.navigate(mod.id); });
      }
...
function getModuloIcon(type, done, active) {
  var color = done ? '#2ECC71' : active ? '#2ECC71' : '#4A5D78';
  var icons = {
...
function markModuleComplete(moduleId) {
  var key = 'nexo_modulos_' + Utils.getDataHoje();
  var completed = JSON.parse(localStorage.getItem(key) || '[]');
  if (completed.indexOf(moduleId) < 0) {
...
function buildNavBar(active) {
  ...
  return '<nav class="bottom-nav">' + items.map(function(i) {
    return '<div class="nav-item ' + (i.id === active ? 'active' : '') + '" data-page="' + i.id + '" onclick="Router.navigate(\'' + i.id + '\')">' + i.icon + '<span>' + i.label + '</span></div>';
  }).join('') + '</nav>';
}
